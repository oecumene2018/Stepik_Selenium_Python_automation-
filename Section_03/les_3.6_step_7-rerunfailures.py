...
# Для PyTest написано большое количество плагинов (https://docs.pytest.org/en/latest/plugins.html),
...
# когда будете писать end-to-end тесты на Selenium. Flaky-тесты или "мигающие" авто-тесты, т.е. такие тесты,
# которые по независящим от нас внешним обстоятельствам или из-за трудновоспроизводимых багов, могут иногда падать,
# хотя всё остальное время они проходят успешно. Это может происходить в момент прохождения тестов из-за
# одновременного обновления сайта, из-за сетевых проблем или странных стечений обстоятельств.
...
# то мы бы увидели сообщение: "2 passed, 1 rerun in 9.20s", и итоговый результат запуска
# всех тестов считался бы успешным.
